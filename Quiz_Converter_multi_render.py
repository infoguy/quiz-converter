...
<body>
  <div class="card">
    ...
    <p>Upload a plain-text quiz file and convert it to a Canvas-compatible QTI .zip.</p>

    <div class="dropzone">
      <strong>1.</strong> Choose one or more quiz <code>.txt</code> files (hold Ctrl/Cmd to select multiple).<br>
      <strong>2.</strong> All files convert and download together as a single <code>QTI_Bundle.zip</code>.
    </div>
...
= Term Two -> Definition B</pre>
    ...
    <ol>
      ...
      <li>Preview the quiz before releasing to students.</li>
    </ol>

    <h2>Instructions for Formatting Guide for Questions</h2>
    <p>This tool supports <strong>Multiple Choice</strong>, <strong>True/False</strong>, <strong>Multiple Answers</strong>, <strong>Essay</strong>, <strong>Fill in the Blank</strong>, <strong>Matching</strong>, and <strong>Question Groups</strong>. Use the exact formats shown.</p>
...
def inject_matching_into_zip(zip_bytes: bytes, matching_blocks: List[Dict], non_matching_qnums: List[int]) -> bytes:
    """
...
def run_text2qti_to_bytes(src: Path, original_name: str = "") -> tuple[bytes, str]:
    """
...
def index():
    ...
    with TemporaryDirectory() as td:
        ...
        for uploaded in uploaded_files:
            ...
            try:
                zip_bytes, zip_name = run_text2qti_to_bytes(in_path, uploaded.filename)
